---
name: kibo-ecommerce
description: Patterns for integrating Kibo Commerce's eCommerce product — unified REST + GraphQL APIs, tenant/site/catalog hierarchy, OAuth with `x-vol-tenant`/`x-vol-site` headers, TypeScript SDK, API Extensions (Arc.js), Event Subscriptions, catalog, cart, checkout, B2B, promotions, built-in CMS. Use when building anything that calls Kibo's commerce APIs, builds a headless storefront on Kibo, or integrates Kibo with a PIM, search engine, OMS, or marketplace. Triggers on Kibo, Kibo Commerce, kibocommerce, KiboCommerce, x-vol-tenant, x-vol-site, MasterCatalog, Kibo cart, Kibo checkout, Kibo catalog, Kibo storefront, Kibo B2B, Kibo promotions, API Extensions, Arc.js, Event Subscription, @kibocommerce/rest-sdk, KiboSoftware. Consult before writing code that calls a Kibo eCommerce API, defines an API Extension, registers an Event Subscription, or maps Kibo data into another system.
---
