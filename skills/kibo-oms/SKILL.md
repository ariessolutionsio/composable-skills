---
name: kibo-oms
description: Patterns for integrating Kibo Order Management — fulfillment, order routing, inventory at locations, returns, and shipping. Covers OMS data model, fulfillment workflows, ship-from-store / BOPIS, safety stock and allocations, RMA flow, carrier integration, and the standalone-OMS pattern (Kibo OMS behind Shopify / SFCC / custom storefronts). Use when implementing Kibo OMS, designing how a third-party commerce platform pushes orders into Kibo, building inventory sync, or designing the warehouse/store/location model. Triggers on Kibo OMS, Kibo Order Management, Kibo fulfillment, kibo-fulfillment-workflows, order routing, BOPIS Kibo, ship-from-store, fulfiller, fulfillment task, RMA Kibo, Kibo returns, Kibo inventory location, Kibo dropship, Kibo carrier, agentic order routing, standalone OMS. Consult before writing code that pushes orders into Kibo OMS, reads inventory from Kibo, defines a fulfillment workflow, or wires up routing rules.
---
